...
: q(static_cast<size_t>(mb.nrJoints())), alpha(static_cast<size_t>(mb.nrJoints())),
  ...
  bodyAccB(static_cast<size_t>(mb.nrBodies())), gravity(0., 9.81, 0.), com(sva::PTransformd::Identity()),
  comVel(sva::MotionVecd::Zero()), comAcc(sva::MotionVecd::Zero())
{
...
ConfigConverter * ConfigConverter::sConstructor(const MultiBody & from, const MultiBody & to)
{
  ...
  if(from.nrBodies() != to.nrBodies()) isOk = false;

  if(from.nrJoints() != to.nrJoints()) isOk = false;
...
void ConfigConverter::sConvert(const MultiBodyConfig & from, MultiBodyConfig & to) const
{
  ...
  if(from.q.size() != to.q.size()) isOk = false;

  if(from.alpha.size() != to.alpha.size()) isOk = false;

  if(from.alphaD.size() != to.alphaD.size()) isOk = false;

  if(from.force.size() != to.force.size()) isOk = false;
...
Eigen::VectorXd sDofToVector(const MultiBody & mb, const std::vector<std::vector<double>> & v)
{
  ...
  for(int i = 0; i < mb.nrJoints(); ++i)
  {
    ...
    {
      std::ostringstream str;
      str << "Dof of joint " << i << " mismatch: expected size " << mb.joint(i).dof() << " gived " << v[ui].size();
      throw std::out_of_range(str.str());
    }
  }
